...
This project is **ASP.NET Core MVC** with **Razor** (not a separate React SPA). AI features are integrated via **JSON API controllers** under `/api/ai/...` and a **floating chat widget** rendered in `_Layout.cshtml`.
...
3. **Database**
   ...
   This creates `AiConversations` and `AiChatMessages` tables for chat history.
...
| POST | `/api/ai/reports/export` | Body: `{ "reportType": "fee_defaulters", "format": "xlsx" }` or `{ "reportType": "executive_summary", "format": "pdf" }` |

## UI

- **Floating assistant** (Admins only): `_AiChatAssistant.cshtml` + `wwwroot/js/ai-assistant.js` (uses `localStorage.token` or `jwt_token` cookie for JWT auth).
